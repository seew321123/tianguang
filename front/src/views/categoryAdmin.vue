<template>
  <div class="about">
    <header class="header">
      <el-button class="newButton" style="height:40px;" @click="newCategory">新建</el-button>
    </header>
    <div v-for="(item,index) in list" v-bind:key="index">
      <div class="category_th" v-if="index==0">
        <div style="width:150px;">分类ID</div>
        <div style="width:300px;">分类名称</div>
        <div style="width:300px;">分类图片</div>
    ...
    <el-dialog title="新建分类" :visible.sync="show_form" @close="hideForm">
      <el-form ref="form" label-width="80px">
        ...
        <el-form-item>
          <el-button @click="upload_category">确认提交</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
  </div>
...
export default {
  ...
  data: function () {
    return {
      ...
      pageSize: 10,
      total_count: 0,
      load_category_url: "/api/load_category",
      receive_image_url: "/api/receive_image",
      upload_category_url: "/api/add_category/",
      delete_category_url: "/api/delete_category/",
      edit_category_url: "/api/edit_category/",
      list: [],
      show_form: false,
      fileList: [],
      formData: {
  ...
  methods: {
    ...
    load_category() {
      ...
      _this.$axios
        .get(_this.load_category_url, { params: data }) // 还可以直接把参数拼接在url后边
        .then(function (res) {
          console.log("res", res)
    ...
    upload_category() {
      ...
      if (this.currentItem) {
        ...
        _this.$axios
          .post(_this.edit_category_url, params) // 还可以直接把参数拼接在url后边
          .then(function (res) {
            console.log("res", res);
          })
          .catch(function (error) {
        ...
        _this.$axios
          .post(_this.upload_category_url, params) // 还可以直接把参数拼接在url后边
          .then(function (res) {
            console.log("res", res);
          })
          .catch(function (error) {
    ...
    delete_category(e) {
      ...
      _this.$axios
        .get(_this.delete_category_url, { params: data }) // 还可以直接把参数拼接在url后边
        .then(function (res) {
          console.log("res", res);
        })
        .catch(function (error) {
